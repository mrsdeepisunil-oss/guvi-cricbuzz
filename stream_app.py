...
# ---------------- CRUD OPERATIONS ----------------
    ...
    with tab4:
        ...
        if st.button("Delete Player", type="primary"): # 'primary' makes the button red/prominent
            if delete_id.strip() == "":
                ...
                try:
                    ...
                    # Check how many rows were actually removed
                    if cur.rowcount > 0:
                        conn.commit()
                        st.success(f"Player ID {delete_id} deleted successfully!")
                        # Optional: Force a rerun to update the 'Read' table
                        st.rerun() 
                    else:
